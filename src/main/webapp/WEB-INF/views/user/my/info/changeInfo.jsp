					...
					<div class="shadow-sm col-md-12 card p-4 pb-5 border-0 ">
						...
						<div class="d-flex justify-content-center">
							<div class="col-md-7 card p-4 pb-5 border-0 mt-4">
								<form
									action="${pageContext.request.contextPath }/user/my/changeInfo-action"
									method="post" id="changeInfoForm">
									<div class="mb-3">
										<label for="userId" class="form-label ms-1">아이디 <span
											class="text-danger">*</span></label>
										<div class="d-flex ">
											<input type="email" class="form-control w-50" id="userId"
												aria-describedby="emailHelp"
												value="${sessionScope.loginUser.userLoginId }"
												disabled="disabled">
										</div>
									</div>
									<div class="mb-3">
										<label for="userPwd" class="form-label ms-1">비밀번호 확인 <span
											class="text-danger">*</span></label> <input type="password"
											class="form-control w-75" id="userPwd" name="userPwd">
										<span class="text-danger ms-1 errMsg">비밀번호를 입력해주세요.</span>
									</div>
									<div class="mb-3">
										<label for="userName" class="form-label ms-1">이름 <span
											class="text-danger">*</span></label> <input type="text"
											class="form-control w-50" id="userName"
											value="${sessionScope.loginUser.userName }"
											disabled="disabled">
									</div>
									<div class="mb-3">
										<label>이메일 <span class="text-danger">*</span></label> <input
											type="email" class="form-control me-1 w-75" id="userEmail"
											value="${sessionScope.loginUser.userEmail}" name="userEmail">
										<span class="text-danger ms-1 errMsg">이메일을 입력해주세요.</span>
									</div>
									<div class="mb-3">
										<label>전화번호 <span class="text-danger">*</span></label>
										<div class="d-flex phoneBox">
											<input type="text" class="form-control me-1 w-25"
												id="userPhone1" maxlength="3"
												value="${fn:substring(sessionScope.loginUser.userPhone, 0, 3)}"
												name="userPhone1"> <span class="mt-2 ms-2 me-2">
												- </span> <input type="text" class="form-control ms-1 w-25"
												id="userPhone2" maxlength="4"
												value="${fn:substring(sessionScope.loginUser.userPhone, 3, 7)}"
												name="userPhone2"> <span class="mt-2 ms-2 me-2">
												- </span> <input type="text" class="form-control ms-1 w-25"
												id="userPhone3" maxlength="4"
												value="${fn:substring(sessionScope.loginUser.userPhone, 7, 11)}"
												name="userPhone3">
										</div>
									...
									<div class="mb-5">
										...
										<div class="d-flex justify-content-start mb-2">
											<input type="text" placeholder="우편번호"
												class="form-control w-50 me-2" id="zipcode" readonly
												onfocus="this.blur();" tabindex="-1"
												value="${sessionScope.loginUser.userZipcode }"
												name="userZipcode">
											<button type="button" class="btn btn-outline-dark ms-2"
												id="addrBtn">주소검색</button>
										</div>
										<div class="mb-2">
											<input type="text" class="form-control" placeholder="기본주소"
												readonly onfocus="this.blur();" tabindex="-1" id="addr1"
												value="${sessionScope.loginUser.userAddress }"
												name="userAddr1" />
										</div>
										<div class="mb-2">
											<input type="text" class="form-control" placeholder="상세주소"
												id="addr2"
												value="${sessionScope.loginUser.userAddressDetail }"
												name="userAddr2" />
										</div>
										<span class="text-danger ms-1 errMsg addrMsg">주소를
											입력해주세요.</span>
									</div>
									<div class="d-flex justify-content-center g-3 mt-4 ">
										<button type="button"
											class="btn btn-dark py-2 me-2 w-25 infoUpdateBtn">정보수정</button>
