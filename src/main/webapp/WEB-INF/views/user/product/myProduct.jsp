				...
				<div class="card shadow-sm border-0 bg-white p-4">
					<div
						...
						<a href="${pageContext.request.contextPath}/product/register"
							class="btn btn-primary btn-sm px-3"> <i class="bi bi-plus-lg"></i>
							새 상품 등록
						</a>
						<!-- </button> -->
					</div>
					<div class="card-body">
						<ul class="nav nav-tabs mb-4 txtColor" id="productTab"
							role="tablist">
							<li class="nav-item"><a
								class="nav-link fw-bold ${param.type == 'ALL' || empty param.type ? 'active' : ''}"
								id="tab-all" data-type="ALL"
								href="${pageContext.request.contextPath }/user/product?type=ALL">전체</a>
							</li>
							<li class="nav-item"><a
								class="nav-link ${param.type == 'PUBLIC' ? 'active' : ''}"
								id="tab-public"
								href="${pageContext.request.contextPath }/user/product?type=PUBLIC"
								data-type="PUBLIC">공개</a></li>
							<li class="nav-item"><a class="nav-link"
								${param.type == 'PRIVATE' ? 'active' : ''} id="tab-private"
								data-type="PRIVATE"
								href="${pageContext.request.contextPath}/user/product?type=PRIVATE">비공개</a>
							</li>
						</ul>
									...
									<c:forEach var="dto" items="${list }" varStatus="status">
										<tr>
											...
											<td class="ps-4">
												<div class="fw-bold">
													<a
														href="${pageContext.request.contextPath }/product/detail?productId=${dto.productId}"
											...
											<td class="text-center"><c:choose>
													<c:when test="${dto.isPublic == 1 }">
														<span
															class="badge bg-success-subtle text-success border border-success px-3">
															공개 </span>
													</c:when>
													<c:otherwise>
														<span
															class="badge bg-danger-subtle text-danger border border-danger px-3">
															비공개 </span>
													</c:otherwise>
												</c:choose></td>
											<td class="text-center text-muted small"><fmt:parseDate
													value="${dto.createdAt}" var="parsedDate"
													pattern="yyyy-MM-dd HH:mm:ss" /> <fmt:formatDate
													value="${parsedDate}" pattern="yyyy-MM-dd" /></td>
											<td class="text-center">
												<div class="d-flex gap-1 justify-content-center">
													<a class="btn btn-secondary btn-sm"
														href="${pageContext.request.contextPath }/product/update?productId=${dto.productId}">수정</a>
													<a class="btn btn-outline-secondary btn-sm"
														href="${pageContext.request.contextPath }/product/delete?productId=${dto.productId}">삭제</a>
													<a
									   ...
									   ${(dto.isPublic != 1 || (not empty dto.auctionId && dto.auctionId != 0)) ? 'disabled' : ''}"
														...
														<c:choose>
															<c:when
																test="${empty dto.auctionId || dto.auctionId == 0}">
									           경매등록
									       </c:when>
