...
.up { margin-top: 50px; border-radius: 7px; border: 1px solid silver; font-size: 16px; padding: 60px !important; }
a, a:visited { color: black !important; text-decoration: none !important; }
h1 { font-size: 20px !important; font-weight: bold !important; }
.box { border: 1px solid silver; height: 100px; width: 90%; border-radius: 7px; position: relative; margin: 30px auto; text-align: center; }
.tot { display: flex; }
.bid { padding: 10px; height: 100%; width: 100%; }
.bid>span { display: block; }
.view { height: 70%; margin-top: 25px; }
.box3 { border: 1px solid silver; border-radius: 7px; width: 90%; text-align: left; padding: 50px; margin: 0 auto; }
.item { vertical-align: top; border-radius: 7px; border: 1px solid silver; margin: 10px; display: inline-block; width: 200px; height: 255px; }
hr { margin: 10px; }
.item span { display: block; }
.item img { border-radius: 5px; object-fit: cover; width: 100%; height: 100%; }
.imgwrap { width: 100%; height: 170px; overflow: hidden; background-color: #f9f9f9; }
.sel { margin-bottom: 20px; width: 850px; display: flex; margin-left: 50px; }
.form-select { width: 150px !important; margin-left: 20px; }
.btn-dark { margin-left: 20px; width: 80px !important; height: 37px !important; font-size: 15px; font-weight: bold; padding: 1px; }
/* ... (기타 기존 스타일 유지) ... */
</style>
					...
					<div class="body">
						...
						<div class="sel">
							...
							<div class="d-flex align-items-center ms-3">
								<input type="text" class="form-control" placeholder="First day" style="width:150px;">
								<span class="mx-2">~</span>
								<input type="text" class="form-control" placeholder="Last day" style="width:150px;">
												...
												<c:choose>
													...
													<c:when test="${bid.shipping == 'Y'}">
														<span id="confirmContainer_${bid.winnerBidId}">
															<button type="button" class="btn btn-light btn-sm" onclick="con(${bid.winnerUserId},${bid.winnerBidId})">구매확정</button>
														</span>
													</c:when>
													<c:when test="${bid.paymentStat == 'Completed' && bid.shipping == 'N'}">
														<span>배송대기</span>
													</c:when>
													<c:when test="${bid.paymentStat == 'Pending'}">
														<a href="${pageContext.request.contextPath}/payment.detail?userId=${bid.winnerUserId}&resultId=${bid.winnerBidId}">
