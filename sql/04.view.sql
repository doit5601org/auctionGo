...
SELECT
    ...
    --TO_CHAR(FN_GET_AUCTION_DUE_DATE(AR.AUCTION_ID),'YYYY-MM-DD HH24:MI') AS AUCTION_END_DATE,
    TO_DATE(SUBSTR(FN_GET_AUCTION_DUE_DATE(AR.AUCTION_ID), 1, 16), 'YYYY-MM-DD HH24:MI') AS AUCTION_END_DATE,
    CASE 
        WHEN EXISTS (
            SELECT 1 FROM AUCTION_CANCEL_HISTORY 
...
CREATE OR REPLACE VIEW VW_UNPAID_WINNING_TARGET_1
AS
SELECT 
     A.AUCTION_ID          AS "경매번호"
    ,A.AUCTION_TITLE       AS "경매제목"
...
SELECT 
    ...
    -- 가입일: 계정 생성 이벤트(보통 1번 코드)의 최소 날짜를 가져오되, 없으면 SYSDATE
    ...
    , CASE 
        WHEN EXISTS (
            ...
            WHERE PH.USER_ID = U.USER_ID
              AND CURRENT_DATE BETWEEN PS.PENALTY_START_DATE AND PS.PENALTY_END_DATE
        ) THEN '제재중'
        ELSE '정상'
      END AS USERSTATUS            
FROM USERS U
JOIN USER_ACCOUNT A ON U.USER_ID = A.USER_ID  -- 계정이 있는 회원만
JOIN USER_PROFILE P ON U.USER_ID = P.USER_ID  -- 프로필이 있는 회원만
WHERE NOT EXISTS (
    -- 탈퇴 회원 테이블(DELETED_USER)에 있는 회원은 제외
    SELECT 1 
    FROM DELETED_USER D 
...
-- ○ 14. 관리자 패널티 조회 뷰
CREATE OR REPLACE VIEW VIEW_PENALTY_LIST
AS
WITH RAW_HISTORY AS (
    SELECT 
        PH.USER_ID, PH.PENALTY_ID, PH.PENALTY_SCORE, PH.CREATED_AT,
        -- 취소되지 않은 제재 점수만 누적 계산
        SUM(CASE WHEN PC.PENALTY_ID IS NULL THEN PH.PENALTY_SCORE ELSE 0 END) 
            OVER (PARTITION BY PH.USER_ID ORDER BY PH.CREATED_AT ASC) AS ACCUMULATED_SCORE,
        PC.PENALTY_ID AS CANCEL_ID, PC.CANCEL_REASON
    FROM PENALTY_HISTORY PH
...
SELECT 
    ...
    -- 누적 점수 4점 이상이면 '영구 정지'로 표시
    CASE 
        WHEN RH.ACCUMULATED_SCORE >= 4 THEN '영구 정지'
        ELSE '제재' 
    END AS PENALTY_STATUS, 

    TO_CHAR(PS.PENALTY_START_DATE, 'YYYY-MM-DD') AS START_DATE,
    
    --  누적 점수 4점 이상이면 종료일을 9999-12-31로 표시
    CASE 
        WHEN RH.ACCUMULATED_SCORE >= 4 THEN '9999-12-31'
        ELSE TO_CHAR(PS.PENALTY_END_DATE, 'YYYY-MM-DD')
    END AS END_DATE,
    
    -- 진행 상태 판별
    CASE 
        WHEN RH.CANCEL_ID IS NOT NULL THEN '취소완료'
        WHEN RH.ACCUMULATED_SCORE >= 4 THEN '제재 중' -- 영구 정지는 항상 제재 중
        WHEN PS.PENALTY_END_DATE < SYSDATE THEN '기간만료'
        ELSE '제재 중'
    END AS HISTORY_STATUS
FROM RAW_HISTORY RH
